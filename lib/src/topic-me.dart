...
class TopicMe extends Topic {
  ...
  void processMetaDesc(TopicDescription desc) {
    ...
    var turnOff =
        (desc.acs != null && !desc.acs!.isPresencer(null)) &&
  ...
  void routePres(PresMessage pres) {
    ...
    if (cont != null) {
      switch (pres.what) {
        ...
        default:
          _loggerService.log(
            "Unsupported presence update in 'me' " + (pres.what ?? ''),
          );
      }
      ...
      if (pres.what == 'acs') {
        ...
        if (acs.mode == INVALID) {
          ...
          _loggerService.warn(
            'Removing non-existent subscription ' +
                (pres.src ?? '') +
                ' ' +
                pres.dacs.toString(),
          );
  ...
  String serializeCreds() {
    return json.encode(_credentials);
  }

  void deserializeCreds(String data) {
    List<dynamic> jsonList = json.decode(data);
    _credentials = jsonList.map((json) => Credential.fromJson(json)).toList();
  }
